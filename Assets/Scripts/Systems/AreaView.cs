...
public class AreaView : MonoBehaviour
{
    ...
    private Dictionary<Node, NodeView> nodeViewsDict; //usar esto mejor para mostrar cosas
    ...
    public void Init()
    {
        ...
        foreach (var nv in nodeViews) nodeViewsDict.Add(nv.node, nv);
    ...
    public static void OnNodeEnter(Node node, NodeView nodeView)
    {
        ...
        if (node.occupant is BoardAgent) HoverPanelView.Populate((BoardAgent) node.occupant);
    }
    ...
    {
        foreach (var item in instance.nodeViewsDict) item.Value.RestoreColor();
        foreach (var node in instance.rangeNodes) instance.nodeViewsDict[node].SetColor(Color.green);
        foreach (var node in instance.areaNodes) instance.nodeViewsDict[node].SetColor(Color.magenta);
    }
    ...
    {
        foreach (var nodeView in instance.nodeViews)
            if (startingNodes.Contains(nodeView.node))
            {
                var _propBlock = new MaterialPropertyBlock();
                var _renderer = nodeView.gameObject.GetComponent<Renderer>();
